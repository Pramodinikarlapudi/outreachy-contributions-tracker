...
xgb = XGBClassifier()
xgb.load_model("/mnt/d/outreachy-contributions-tracker/OtherFeaturizersAnalysis/hERG_xgb_model.json")  # Adjust if needed
...
y_test = chembl_data['LABEL (Y)'].astype(int).values  # Your manual labels
...
plt.legend()
plt.savefig('/mnt/d/outreachy-contributions-tracker/figures/hERG_ChEMBL_xgb_roc_curve.png')
plt.close()
...
plt.savefig('/mnt/d/outreachy-contributions-tracker/figures/hERG_ChEMBL_xgb_confusion_matrix.png')
plt.close()

#doublecheck
chembl_data['Predicted_Label'] = y_pred
chembl_data.to_csv("/mnt/d/outreachy-contributions-tracker/ChEMBL_hERG/hERG_chembl_50_predictions_xgb.csv", index=False)
